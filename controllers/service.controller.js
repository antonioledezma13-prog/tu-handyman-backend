...
// Cotizar = consume 1 lead del técnico
exports.quoteService = async (req, res) => {
  try {
    ...
    // Cargar técnico y verificar leads
    const tech = await User.findById(req.user._id);
    if (!tech) return res.status(404).json({ error: 'Técnico no encontrado.' });

    const hasLeads = await tech.hasLeadsAvailable();
    if (!hasLeads) {
      const remaining = await tech.leadsRemaining();
      return res.status(403).json({
        error:           'Sin leads disponibles este mes. Mejora tu plan para continuar.',
    ...
    // Consumir lead después de cotizar exitosamente
    await tech.consumeLead();
    ...
    const leadsRemaining = await tech.leadsRemaining();
    res.json({ service, leadsRemaining, leadsUsed: tech.leadsUsed });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
